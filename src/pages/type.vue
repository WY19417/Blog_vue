<template>
	<div class="tag">
		<el-card class="box-card">
			...
			<div class="text item">
				<el-tag size="mini" class="tag-item" v-for="item in type"  :key="item.id" :value="item.name" @click="change(item.name)">{{item.name}}<span>[{{item.blogs.length}}]</span></el-tag>
			</div>
		</el-card>


		<el-col :span="24" style="margin-top: 31px;" v-model="type">
			<h3>这是{{typetitle}}种类下的博客</h3>
        <el-row class="art-item">
          <el-card shadow="hover" v-for="item in typemsg" :key="item">
            <h5>
		...
		created(){
			const _this = this
			 axios.get('http://localhost:8081/type/findAll').then(function(resp){
				...
				_this.typemsg = resp.data[0].blogs
			 })
		},
		methods: {
			change(name){
				for(var i in this.type){
					if(this.type[i].name == name){
						this.typetitle = this.type[i].name,
						this.typemsg = this.type[i].blogs
					}
				}
			}
		}
