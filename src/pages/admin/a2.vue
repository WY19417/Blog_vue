    ...
    methods: {
      handleEdit(index, row) {
        this.dialogFormVisible = true;
          ...
          console.log(row);
          axios
        .get("http://localhost:8081/tag/find/" + row.id)
        .then(function(resp) {
          _this.ruleForm = resp.data;
        });
      },
      handleDelete(index, row) {
        axios.delete("http://localhost:8081/tag/delete?id=" + row.id).then(function(resp){
              alert("删除成功");
      ...
      send(input){
        axios.get('http://localhost:8081/tag/find/'+input).then(function(resp){
          // _this.tableData = resp.data
          // this.$forceUpdate()
      ...
      add(msg){
        const _this = this
        axios.put('http://localhost:8081/tag/save?name='+msg.name).then(function(resp){
